...
export default function ResumeEditorPage() {
  ...
  const handleAiSummary = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/ai/summary`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token || 'mock-dev-token'}`
  ...
  const handleAiPolish = async (index: number) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/ai/grammar`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token || 'mock-dev-token'}`
          ...
          <div style={{ position: 'sticky', top: '2rem', height: 'calc(100vh - 4rem)', overflowY: 'auto', paddingRight: '0.5rem' }} className="preview-pane">
            <ResumePreview 
              ...
              onPrint={async () => {
                try {
                  const token = await getIdToken();
                  await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/pdf/generate`, {
                    method: 'POST',
                    headers: {
                      'Content-Type': 'application/json',
                      'Authorization': `Bearer ${token || 'mock-dev-token'}`
